...
EditorConfig 有助于为不同 IDE 编辑器上处理同一项目的多个开发人员维护一致的编码风格。(创建.editorconfig 文件)
...
- 方式一：直接通过 CLI 提供给我们的选项来配置(和 webpack 属性名不一样,不过是和 webpack 属性名一一对应的)：
  - 比如 publicPath：配置应用程序部署的子目录（默认是 `/`，相当于部署在 `https://www.my-app.com/`）；
  - 比如 outputDir：修改输出的文件夹；
- 方式二：通过 configureWebpack 修改 webpack 的配置(和 webpack 属性名完全一致)：
  - 可以是一个对象，直接会被合并；
